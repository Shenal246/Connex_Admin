...
const StaffLogs = () => {
    ...
    const [staffUserId, setStaffUserId] = useState(''); // State to hold the staff user ID filter
    ...
    const fetchstaffLogsApi = APIConnection.fetchstaffLogsApi;

    // Fetch staff logs data
    const fetchLogDetails = async () => {
        try {
            const response = await axios.get(fetchstaffLogsApi, { withCredentials: true });
    ...
    const filteredLogs = logData.filter((log) => {
        ...
        const staffIdMatch = log.staffName.toLowerCase().includes(staffUserId.toLowerCase());
        const fromDateMatch = fromDate ? new Date(log.timestamp) >= new Date(fromDate) : true;

        return (actionMatch || timestampMatch) && staffIdMatch && fromDateMatch;
    });
                        ...
                        <Table stickyHeader>
                            <TableHead>
                                <TableRow>
                                    ...
                                    <TableCell sx={{ backgroundColor: '#444444', color: '#ffffff', fontSize: '0.875rem' }}>
                                        <Typography variant="subtitle2" sx={{ color: '#ffffff' }}>Staff Name</Typography>
                                    </TableCell>
                                </TableRow>
                            </TableHead>
                            <TableBody>
                                {filteredLogs.map((log, index) => (
                                    <TableRow key={log.logid} onClick={() => handleRowClick(log)} sx={{
                                        ...
                                        <TableCell>{log.staffName} ({log.employeeID}) </TableCell>
                                    </TableRow>
                                ))}
                        ...
                        <DialogContent dividers>
                            {selectedLog && (
                                <>
                                    ...
                                    <Typography variant="body1"><strong>Staff Member:</strong> {selectedLog.staffName} ({selectedLog.employeeID})</Typography>
                                </>
                            )}
                        </DialogContent>
                        <DialogActions>
